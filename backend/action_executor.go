...
func (e *ActionExecutor) Execute(ctx context.Context, action WebAction) WebAction {
    ...
    if action.Kind=="provider.health" && e.adapters!=nil {
        err:=e.adapters.ExecuteHealth(ctx,strings.ToLower(action.Target))
        if err!=nil { action.Status="failed"; action.Error=err.Error(); e.record(action,"failed","provider health operation failed"); return action }
        action.Status="completed"; e.record(action,"completed","provider health operation executed by server-side adapter"); return action
    }
