...
func (g *GitHubAdapter) DispatchWorkflow(ctx context.Context,workflow string,inputs map[string]string)error{payload:=map[string]any{"ref":g.Branch,"inputs":inputs};body,err:=json.Marshal(payload);if err!=nil{return err};_,_,err=g.request(ctx,http.MethodPost,"/repos/"+g.Owner+"/"+g.Repo+"/actions/workflows/"+workflow+"/dispatches",strings.NewReader(string(body)));return err}
